...
def IPv6_Append_Function(IPv6Prefixlist_PrefixID_list,Final_append_Dictlist):
    max_count = len(IPv6Prefixlist_PrefixID_list)
    ...
    for Append_entries in Final_append_Dictlist:
        ...
        count=1
        for ID in IPv6Prefixlist_PrefixID_list:
            print("------------------------------------------------------------------------")
            print("Special Notification : Just FYI : Preparing to add the IP " + IP + " to the prefix list : ", ID)
            Append_Loop_IPv6Prefixlist_Entries= []
            Append_Loop_IPv6Prefixlist_Entries_finallist = []
            Append_Loop_IPv6Response = pfxclinet.describe_managed_prefix_lists(PrefixListIds = [ID])
            Append_Loop_IPv6Prefixlist=Append_Loop_IPv6Response.get('PrefixLists')
            for x in Append_Loop_IPv6Prefixlist:
                Append_Loop_IPv6Prefixlist_Version = x["Version"]
                Append_Loop_IPv6Prefixlist_State = x["State"]
                Append_Loop_IPv6Prefixlist_MaxEntries = x["MaxEntries"]
            Append_Loop_IPv6Prefixlist_Metadata = paginator.paginate(PrefixListId=ID)
            for i in Append_Loop_IPv6Prefixlist_Metadata:
                Append_Loop_IPv6Prefixlist_Entries = (i["Entries"])
                #print("Append_Loop_IPv6Prefixlist_Entries ",Append_Loop_IPv6Prefixlist_Entries)
                for Entries in Append_Loop_IPv6Prefixlist_Entries:
                    Append_Loop_IPv6Prefixlist_Entries_finallist.append(Entries.get("Cidr"))

            print("Special Notification : Just FYI : The prefix list " + ID +" and its version " ,Append_Loop_IPv6Prefixlist_Version, "has a total entry of ",len(Append_Loop_IPv6Prefixlist_Entries_finallist)," and can accomdate max entries of: ", Append_Loop_IPv6Prefixlist_MaxEntries)

            if (len(Append_Loop_IPv6Prefixlist_Entries_finallist) < Append_Loop_IPv6Prefixlist_MaxEntries):
                print("Special Notification : Just FYI : Adding the IP " + IP + " to the prefix list : ", ID)
                try:
                    IPv6PrefixlistUpdate_Response = pfxclinet.modify_managed_prefix_list( CurrentVersion= Append_Loop_IPv6Prefixlist_Version, PrefixListId=ID, AddEntries= [Append_entries])
...
def IPv6_Remove_Function(IPv6Prefixlist_PrefixID_list,removal_list):
    print("Validation 5 : Initiating : Commencing IP removal process for all the IP's : ", removal_list ) 
    for ID in IPv6Prefixlist_PrefixID_list:
        Pop_Loop_IPv6Prefixlist_Entries= []
        Pop_Loop_IPv6Prefixlist_Entries_finallist = []
        Pop_Loop_pop_list = []
        Final_removal_Dictlist =[]
        Pop_Loop_IPv6Response = pfxclinet.describe_managed_prefix_lists(PrefixListIds = [ID])
        Pop_Loop_IPv6Prefixlist=Pop_Loop_IPv6Response.get('PrefixLists')
        for x in Pop_Loop_IPv6Prefixlist:
            Pop_Loop_IPv6Prefixlist_Version = x["Version"]
            Pop_Loop_IPv6Prefixlist_State = x["State"]
        Pop_Loop_IPv6Prefixlist_Metadata = paginator.paginate(PrefixListId=ID)
        for i in Pop_Loop_IPv6Prefixlist_Metadata:
            Pop_Loop_IPv6Prefixlist_Entries = (i["Entries"])
            #print("Pop_Loop_IPv6Prefixlist_Entries ",Pop_Loop_IPv6Prefixlist_Entries)
            for Entries in Pop_Loop_IPv6Prefixlist_Entries:
                Pop_Loop_IPv6Prefixlist_Entries_finallist.append(Entries.get("Cidr"))
        ...
        Pop_Loop_pop_list = set(Pop_Loop_IPv6Prefixlist_Entries_finallist) - (set(Pop_Loop_IPv6Prefixlist_Entries_finallist) - set(removal_list))
        if len(Pop_Loop_pop_list) != 0:
            ...
            try:
                IPv6PrefixlistUpdate_Response = pfxclinet.modify_managed_prefix_list(
                            CurrentVersion= Pop_Loop_IPv6Prefixlist_Version,
                            PrefixListId=ID,
...
def lambda_handler(event, context):
    ...
    IPv6Response = pfxclinet.describe_managed_prefix_lists(
        Filters=[
        {'Name': 'prefix-list-name',
         'Values': ['*AUTOMATED_PREFIXLIST_IPV6*']}
         ])

    ### Note : Test the script with 1000 IP entries in the prefix list to make sure it is able to manage all the 1000 entries.
    IPv6Prefixlist_Entries_intermediatelist= []
    IPv6Prefixlist_Entries_finallist = []
    IPv6Prefixlist_PrefixID_list = []

    IPv6Prefixlist=IPv6Response.get('PrefixLists')
    for Metadata in IPv6Prefixlist:
        IPv6Prefixlist_Version = Metadata["Version"]
        IPv6Prefixlist_State = Metadata["State"]
        IPv6Prefixlist_PrefixID = Metadata["PrefixListId"]
        #This is to collect all the prefixID
        IPv6Prefixlist_PrefixID_list.append(IPv6Prefixlist_PrefixID)
        IPv6Prefixlist_Entries = paginator.paginate(PrefixListId=IPv6Prefixlist_PrefixID)
        for i in IPv6Prefixlist_Entries:
            IPv6Prefixlist_Entries_intermediatelist.append(i["Entries"])
            #print(IPv6Prefixlist_Entries_intermediatelist)
    #The beow is applicable because, the above output has list with 2 square brackets
    for Entries in IPv6Prefixlist_Entries_intermediatelist:
        for SubEntries in Entries:
            IPv6Prefixlist_Entries_finallist.append(SubEntries.get("Cidr"))
    ...
    #print(IPv6Prefixlist_Entries_finallist)
    
    print("Validation 1 : Passed : Step 1 completed and all CIDR's has been extracted from the IPv6 prefix list for comparision.")
    ...
    for SSMmetadata in AllSecretlist:
        ...
        try:
            SecretIPList = SecretKeystring["IPv6"].split(',')
        except:
            print("Special Notification : Just FYI : No IPv6 address are avaliable for the user " + SecretName + ". Kindly check with admin team regarding the same.")
            continue
        ...
        for IP in SecretIPList:
            ...
            if IP in IPv6Prefixlist_Entries_finallist:
    ...
    print("Validation 2 : Passed : Step 2 completed and all IPv6 CIDR's has been extracted from the secretmanager for comparision.")
    ...
    removal_list = set(IPv6Prefixlist_Entries_finallist) - set(Full_secretmanager_iplist)
    
    
    print("Validation 3 : Passed : The list of IPv6 CIDR's that will be added to the prefix list as part of the run : ",  Final_append_list )
    
    
    print("Validation 4 : Passed : The list of IPv6 CIDR's that will be removed from the prefix list as part of the run : ",  removal_list )
    ...
    if len(removal_list) != 0 :
        IPv6_Remove_Function(IPv6Prefixlist_PrefixID_list,removal_list)
    else:
        print("Validation 5 : Passed : No IPv6 CIDR's in the removal list. Hence the prefix list is not altered as part of the removal process." )
        
    ### IP addition part ###
    if len(Final_append_list) != 0:
        IPv6_Append_Function(IPv6Prefixlist_PrefixID_list,Final_append_Dictlist)
    else : 
        print("Validation 6 : Passed : No IPv6 CIDR's in the append list. Hence the prefix list is not altered to add any IP's." )
